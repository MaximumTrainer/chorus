...
Payload keys distinguish the template from what was sent: `promptTemplateId` / `promptTemplateVersion` / `promptTemplateHash` for provenance, and `prompt` / `promptHash` / `promptLength` for the body the redaction policy retained. A single `promptHash` meaning "the template" in some runs and "what we sent" in others is a trace nobody can query without reading the code that wrote it.
