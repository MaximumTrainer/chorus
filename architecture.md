...
- Cross-artefact relationships are expressed once, in `artefact_links(from_type, from_id, to_type, to_id, relation, detail jsonb)` — `detail` carries what makes a link checkable, such as the document sections a task was derived from (DOC-6 AC2), rather than as bespoke foreign keys per pair. Foreign keys are reserved for containment (`tasks.parent_id`, `messages.session_id`).
...
**`api`, `worker` and `collab` share one image with three commands.** They scale independently, which is why they are separate processes, but they must never run *different code*: a worker on a different commit from the API that enqueued to it produces a class of bug that is very hard to see. Two Dockerfiles would mean two builds of one monorepo and two chances to drift.

**The API answers a browser on another origin only where it was told to (DOC-2).** Behind a single reverse proxy the web app and the API share an origin and none of this applies; on separate hosts — which is every development machine — the browser will not let the page call the API without being told it may. `CHORUS_WEB_ORIGINS` is an allow-list, **empty by default**, and the response names the allowed origin rather than answering `*`: a wildcard and credentials are mutually exclusive in the specification precisely because the combination would let any page on the internet use somebody's session cookie. Preflights are answered before authentication, since a preflight carries no credentials by design — authenticating one returns 401, the browser never sends the real request, and the symptom is a feature that silently does nothing.

**An export resolves its links against the deployment it left (DOC-7 AC4).** A relative path means something only inside the app it came from; exported as-is it resolves against whatever document the reader happens to be looking at, which is worse than a link that is visibly broken. So both serialisers take the base URL, and both apply it to image sources as well as links — a document that arrives complete with every diagram a broken icon is not an export. External URLs are left exactly as written.

Rich text is emitted as **HTML with the structures a word processor actually reads**: a real `<table>` with `<th>` and `<td>`, a real `<ul>`, a real `<pre><code>`. A paste that arrives as styled paragraphs has lost the thing somebody wanted the table for. Every piece of content is escaped without exception, because a document about software eventually contains `<` and `&&`, and an unescaped one opens a tag the rest of the export then lives inside. The response carries `text/html`, since that is how a word processor decides what a paste is.

**A restore is additive, and captures what it replaces (DOC-5).** Nothing in the history is ever updated in place: restoring writes a *new* version, and the state it replaced is written as one too. That second snapshot is the part that is easy to leave out and expensive to miss — nobody snapshots the document they are about to lose, so without it a restore is itself irreversible, which is precisely the "losing work twice" the requirement names. A restore may carry the `updated_at` the client was looking at; if the document moved in between, it is **refused** rather than merged, because a document that is half one version and half another is an outcome nobody asked for and nobody can describe afterwards.

Diffs are computed over **rendered Markdown at block granularity**, not over the CRDT's operation log — an operation log is a faithful record of how a document got here and is unreadable as an answer to "what changed". Each version stores its own rendering rather than re-rendering the snapshot on demand, so an old diff does not change when the renderer does. Lists and tables are compared row by row: treating a table as one block reports a single changed cell as the whole table being replaced. A block that moved is reported **once, as a move**, rather than as a deletion plus an insertion, because reordering is most of what editing a specification consists of and a diff that doubles every reorder is one people learn to skim. Retention prunes ordinary snapshots and never an approval or a restore: those are the evidence of a decision and the record of an undo.

**A comment is anchored to the text it quotes, not to a position (DOC-4).** A position has to be rebased against every edit anybody makes before it, and gets that wrong silently; a quotation is located afresh whenever the thread is read, so an edit elsewhere in the document cannot move it at all. What that costs is the case where the same words appear twice, which is what the stored prefix is for. The quotation is kept whatever happens to the document, because it is the only way to render an orphan usefully — "this was about *this sentence*, which is gone" is something a reader can act on.

There is deliberately **no `orphaned` column**. Whether a thread still has its text is a fact about the document as it is now; a stored flag would be a second answer to that question, correct until somebody edits the document through a path that forgot to update it. A comment on text the document does not contain is refused at the point of writing rather than accepted as an instant orphan — the author is still there and can point at something real. Resolving is a state change and never a deletion: the reasoning is the part worth keeping, and it is what somebody reopening the thread needs to read.

A **mention of a non-member is refused**, not dropped, for the same reason a lost comment matters: somebody believes a colleague was asked. Notifications are sent *after* the transaction commits — one sent inside it can be delivered for a comment that then fails to save, and the recipient follows a link to nothing. DOC-4 also made `path` a stored column on `notifications`: it had been used only to render a link into the email, so the in-app copy of a mention told somebody they were wanted and not where.

**A suggestion lives beside the document, never inside it (DOC-3).** Until somebody accepts one, the document is byte-identical to what it was — through every path a reader has: the API, the export, MCP. That is a property of *where the data is* rather than of how carefully each reader behaves, which is the point. The alternative, marks inside the CRDT, is what the requirement's implementation note suggests and what the editor will eventually render; it is not where the data lives, because text inside the document has to be filtered out by every consumer — export, prompt builder, wiki compiler, search indexer — and a guarantee that holds only while everyone remembers to filter is not a guarantee.

A suggestion is anchored by **the text it replaces**, not by a position. A position goes stale the moment anyone types anywhere before it; the text goes stale only when the text itself changes, which is exactly when a suggestion should stop applying (AC5). Applying requires the quotation to appear *exactly once*: none means the document moved on, more than one means there is no way to know which was meant, and both are cases where doing nothing beats guessing — the person accepting believes they know what they agreed to. Two filters run at generation rather than at acceptance, because a suggestion that cannot apply should never be offered: one outside the selection would edit text nobody offered up (AC3), and models invent quotations that match nothing.

**A stale mark is committed before the error that reports it.** Raising the conflict inside the same transaction rolls the mark back, so the caller is told the suggestion is stale and the next reader is offered it again. The first implementation did exactly that, and the test that caught it asserts on the stored status rather than on the response.

**A document has one body, and it is the CRDT (DOC-2).** DOC-1 shipped documents as template `sections` carrying their own content; DOC-2 gave them a `ydoc`. Two representations meant two answers to "what does this document say", and the one that leaves the building — the export, the prompt, the wiki page — was not the one the editor showed. So the body is the CRDT and only the CRDT. The `sections` column carries **structure alone** (key, title, guidance, required); a section's content is read out of the body, and a section is a *heading inside* the body rather than a container around it — which is what keeps the body one editable surface, where somebody can write across a section boundary or move a paragraph between two. The key travels on the heading's attributes so a section stays addressable after its title is rewritten.

The API still writes: an agent emitting a draft, a section filled from the task panel, text pasted into a new session. Those writes are **diffed into the existing CRDT** rather than replacing it, because a replacement is a different document — merged with what an open editor holds it does not overwrite, it duplicates. And the collaboration server **merges the stored state before writing its own**, since it is not the only writer: storing its in-memory copy alone would silently drop every API write made since it loaded the document. Applying an update is additive and carries its own delete set, so merging cannot resurrect what somebody deleted. `body_md_cache` is written by both paths, because a cache only some writers update is worse than none — right often enough to be trusted, and wrong exactly while somebody is editing.

**One document schema, in `packages/ui` (DOC-2 AC6).** The editor, the state the collaboration server stores, and every export read the same definition of what a document may contain. This matters because a node type the schema does not know is not rejected by ProseMirror — it is *dropped*: the document still opens, still looks like a document, and is quietly missing the table somebody spent an afternoon on. Two definitions of "what a document is" produce exactly that at the boundary between them. The Markdown serialiser is written rather than taken from `prosemirror-markdown`, which has no output for tables, task lists, mentions or embeds — the four node types most likely to carry the part of a document somebody actually needs, and losing them at export is the same loss arriving later and somewhere else.

**How the collaboration channel is authorised (DOC-2 AC5).** A WebSocket cannot carry the session cookie the browser holds, and `collab` is a separate process on its own port. So the browser asks `api` — where every rule about who may read what already lives — for a **collaboration ticket**, and presents that on the socket. The alternative, teaching `collab` to decide for itself whether somebody may read a document, means two implementations of one permission question; they drift, and the one that drifts is the one nobody is looking at.

A ticket therefore carries no authority of its own: it is a receipt for a decision `api` already made, naming exactly one document, valid for thirty seconds. Two things about it were got wrong first and are worth recording. It is **not single-use** — that was the original design and does not survive the protocol, because a client authenticates more than once while establishing a connection and a strictly single-use ticket refuses its own rightful holder mid-handshake; the protection is the lifetime. And revocation **expires the ticket before closing the socket**, because a client treats a closed connection as something to retry: closing alone makes losing access look like a network blip that lasts about a second. Open connections are re-checked on an interval, so revoking access is felt in the session it happens in rather than at the next reload.
...
**Work**
`chat_sessions(team_id, title, entry_point, routing_hint, surface, external_thread_ref)` — named `chat_sessions` rather than `sessions` because the authentication library is configured with `modelName: 'sessions'` and creates that table itself at runtime, so a migration claiming the name fails on any database that has been started once. The domain word stays "session" everywhere a person sees it; only the table carries the qualifier, `messages(session_id, role, author_user_id, run_id, content jsonb, context_used jsonb)`, `documents(team_id, type, template_id, ydoc bytea, body_md_cache, status, approved_by, approved_at)`, `document_versions(document_id, snapshot bytea, label, created_by)`, `comments(target_type, target_id, anchor jsonb, thread_id, body, resolved_at)`, `structure_proposals(team_id, session_id, run_id, source_document_id, tree jsonb, confirmed_tree jsonb, status, decided_by, decided_at, feedback)` with `structure_proposal_tasks(proposal_id, task_id, node_key)`, `tasks(team_id, parent_id, key, title, description jsonb, acceptance_criteria jsonb, tags text[], status, priority, size, assignee_id, embedding, position)`, `code_pointers(task_id | capture_id, repo_id, path, symbol, line_start, line_end, commit_sha, source, confidence)`, `artefact_links(from_type, from_id, to_type, to_id, relation)`, `external_links(task_id, integration_id, external_id, external_key, url, sync_state, last_synced_hash, last_synced_at)`, `attachments(target_type, target_id, key, mime, size, name)`.
...
- A `schema_migrations(filename, checksum, applied_at)` ledger records what a database has already seen, and each migration is applied **in the same transaction as its ledger row** — written apart, a crash between the two leaves the ledger lying in one direction or the other. The migrator runs on every start, so "forward-only" means forward from wherever that database already is (NFR-1 AC5). A file whose checksum no longer matches what was applied is **refused rather than skipped**: two deployments claiming one schema version with different schemas is worse than a loud failure. A database with a schema and no ledger predates it and is adopted once, explicitly, with `pnpm db:migrate --baseline` — see [ADR-0016](docs/adr/0016-schema-migration-ledger.md).
...
`stream` carries tool-call deltas as well as text, so a caller sees a call announced, its arguments arrive in fragments, and then the parsed call — deltas rather than whole calls because a coding job's arguments are large (a file edit is a diff) and a panel showing nothing until the call completes reads as a hang. Arguments are parsed once, in the provider: providers differ in how they escape JSON strings, and a consumer string-matching raw fragments breaks on the first escape it has not seen. Both providers key their accumulation on the index every frame carries, because a turn may contain several calls interleaved and an accumulator keyed on anything else merges them into one whose arguments still parse.

Two providers ship: the OpenAI-compatible one and Anthropic. A **registry** dispatches each call to the provider its resolved `{provider, model}` names, and is itself a `ModelProvider`, so no caller learns there is more than one. A provider a tier names but the deployment has not configured is refused, naming what is configured — never served by whichever client is at hand, which would bill the wrong ledger line and reason with the wrong capabilities. Every provider must pass the shared **contract-test kit** in `packages/llm/src/testing/`, which asserts behaviour rather than wire format; a guarantee that holds for one provider and not another is not a guarantee (ADR-0018).
...
- **Every request bounds its output.** Omitting a maximum does not mean "a sensible default"; it means the endpoint chooses, and endpoints choose their model's maximum. Both providers therefore send a conservative ceiling when the caller names none — unbounded output is unbounded cost, and a quota-limited gateway refuses such a request outright rather than trimming it. A caller that needs more says so.
...
Every step that executes emits an event, and no workflow opts in — a hook a workflow has to declare is one the fourth workflow forgets, and an evaluation harness then reports on a subset and looks healthy. `test/…/replay.test.ts` enumerates the run's own steps against its own events rather than checking a fixed list, so a step type added later is covered by the same assertion.

Payload keys distinguish the template from what was sent: `promptTemplateId` / `promptTemplateVersion` / `promptTemplateHash` for provenance, and `prompt` / `promptHash` / `promptLength` for the body the redaction policy retained.

The team charter is injected into every model call as its stable prefix (§6, §10) and recorded as `teamCharter` **beside the structural record, at every redaction level** (WS-3 AC2). That is a deliberate exception, and a narrow one: a charter is configuration an admin wrote and every member can already read through the API, not the customer content §11.6 exists to keep out of traces. Left inside the body it would vanish at the default `structural` level, and "why did the agent do that?" would be unanswerable for the one input that applies to every turn. It is credential-scrubbed like everything else, because free text an admin typed is exactly where a key gets pasted by accident. A single `promptHash` meaning "the template" in some runs and "what we sent" in others is a trace nobody can query without reading the code that wrote it.

**Replay returns what the run kept, and says when that is not enough.** A run at `structural` has no bodies, and rebuilding a prompt from the template plus today's inputs and presenting it as what ran would differ from the original exactly when it matters — after the template changed, after the retrieved context moved on, which is when somebody is replaying in the first place. What remains without the body is still worth having: the template at the version that ran, and a hash of what was sent, so a reconstruction can be *verified* against the original even though the original is not there to read.
...
**The export replays a stored trace; it does not re-time one.** Spans carry the times the work actually happened, because a trace stamped with the moment somebody exported it collapses every historical run onto that moment and is then useful for nothing but counting. A run is one root span with every event beneath it: a collector accepts a flat pile of spans just as readily as a tree, so the nesting is built deliberately or it is not there. An event is a point in time rather than an interval — `run_events` records when a thing happened, not how long it took — so its span is zero-length, which is honest and renders correctly in every backend.

An export with an explicit endpoint gets **its own exporter and its own flush**, rather than borrowing the process's. A stored trace may belong in a different backend from live telemetry, and it must not be silently swallowed by whatever the process happened to be configured with — the first version reused the global provider and its endpoint argument did nothing whenever telemetry was already initialised, which in a deployment is always.

Span attributes are an **allow-list**, not the event payload spread wholesale. A payload carries prompt and response bodies, and putting those into whatever tracing backend a deployment runs is precisely what the redaction policy exists to prevent; the attributes carried are the fields somebody would query a trace *by*.

The compose stack runs a stock OpenTelemetry collector, and the AC5 suite asserts against the collector's own `otelcol_receiver_accepted_spans` counter. An assertion we write about wire format checks our reading of the specification rather than the specification; the collector's opinion is the one the criterion asks for. Nesting and attributes are checked separately, on the spans themselves, because acceptance says nothing about parentage.
...
Code pointers (TASK-3) are the emit-step guarantee applied to a concrete artefact: a pointer is written only after its file and line range have been checked against the index, never validated on read. Three consequences worth stating. A match below the confidence floor produces **no pointer at all**, because a missing pointer costs a reader one search while a wrong one costs the credibility of every other pointer. A pointer whose file has moved is **marked stale rather than deleted**, so the last known good commit still links — an absence tells a reader nothing. And regeneration replaces only `generated` pointers: somebody who corrected one has told us something the index does not know, and discarding it is how a tool teaches people that correcting it is pointless.

Retrieval exposes a normalised `confidence` alongside the raw fusion score. A reciprocal-rank score is a *ranking* device — about 0.016 at the top with the usual constant — so it cannot be compared against any threshold a person would write down; dividing by the best attainable score makes it a number a caller can reason about, while keeping the property that agreement between searches beats a single strong hit.
...
An adapter's standing instructions must not restate what the brief already says. The brief is the authority on the repository, and a second source of truth is one the model follows whichever it read last — a live run made this concrete: an instruction to "run the repository test command", given to an agent whose brief said none was detected, cost four of eight turns hunting for a `package.json` that did not exist, after the actual edit was already done. Instructions that touch the repository are therefore conditional on the brief. Each turn also bounds its own output: a loop makes many calls, and one large ceiling per turn is both wasteful and, on a quota-limited gateway, fatal, because the ceiling is checked against the remaining balance before a token is produced.
...
POST       /workspaces/{id}/proposals/{proposalId}/confirm   materialises the tree into tasks, once (CHAT-5)
POST       /workspaces/{id}/proposals/{proposalId}/reject    with feedback the next turn reads
POST       /workspaces/{id}/documents/{documentId}/decompose  runs decompose-tasks, producing a proposal (DOC-6)
...
- **Performance suite** — retrieval, chat first token and index throughput measured against §24 budgets on a fixed corpus, run nightly with trend reporting. The corpus is **generated, not vendored** (`generateCorpus`): half a million lines committed would slow every clone forever, and a generator can be re-run at a different size when a budget or the hardware moves. It is seeded, so the same input produces byte-identical output — a corpus that varied would turn every regression into an argument about whether the corpus got harder.
  ...
  Retrieval is asserted on the **query plan** as well as the clock, because at a corpus small enough to run in a minute an index lookup and a scan are milliseconds apart while at a million chunks they are a hundredfold apart — no affordable timing threshold separates them, and the planner will say directly which it chose. That makes the plan part of the contract, and a plan is a property of the *server's configuration* as much as of the SQL: `deploy/docker-compose.yml` starts Postgres with a `random_page_cost` chosen because the default made the planner abandon the HNSW index at twenty thousand chunks. CI therefore configures its Postgres from that same compose file (`scripts/reference-db-config.mjs`) rather than declaring the value a second time, and an NFR-12 AC4 suite asserts the server a run is actually using carries it. A setting that only some environments have is how a benchmark comes to pass locally and fail nightly on a plan nobody can reproduce.
...
8. **Realtime for non-document state** — confirm that optimistic updates with ETags suffice for task boards, or promote them to CRDT. Task ordering now uses **fractional keys** (`packages/core/src/ordering.ts`), which makes a move a single-row write: two people reordering different tasks touch different rows and both survive, with no shared counter to race over. That removes the main argument for a CRDT here; what remains open is whether the *presence* and live-cursor experience needs one.
